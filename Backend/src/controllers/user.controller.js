...
const generateAccessAndRefreshTokens = async (userId) => {
    try {
        ...
        await user.save({ validateBeforeSave: false }); //since only refresh token is sent to save it will validate the whole obj and everything except refresh token will be deleted thats why it is false
...
const loginUser = asyncHandler(async (req, res) => {
    //collecting user data from frontend i.e req.body
    ...
    const loggedInUser = await User.findById(user._id).select(
        "-password -refreshToken"
    ); //we can do it in another way without db query
...
const logoutUser = asyncHandler(async (req, res) => {
    ...
    //options allows only server to modify cookies
    const options = {
        httpOnly: true,
        secure: true,
    };
